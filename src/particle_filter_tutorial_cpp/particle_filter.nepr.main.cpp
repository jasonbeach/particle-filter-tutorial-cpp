...
int main() {
  ...
  const auto number_of_effective_particles_threshold = number_of_particles / 4.0;
  ...
  particle_filter_nepr.initialize_particles_uniform();
  ...
  for (size_t ts = 0; ts < n_time_steps; ++ts) {
    ...
    visualizer.draw_world(world, robot, particle_filter_nepr.particles());
