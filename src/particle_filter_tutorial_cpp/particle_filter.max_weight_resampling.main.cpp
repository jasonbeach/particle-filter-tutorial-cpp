...
int main() {
  ...
  const auto resampling_threshold_reciprocal = 1.0 / 0.005;

  ParticleFilterMWR particle_filter_mwr {
    number_of_particles, lp, pnp, mnp, resampling_algorithm, resampling_threshold_reciprocal};
  particle_filter_mwr.initialize_particles_uniform();
  ...
  for (size_t ts = 0; ts < n_time_steps; ++ts) {
    ...
    visualizer.draw_world(world, robot, particle_filter_mwr.particles());
